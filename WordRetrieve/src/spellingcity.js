...
const outputname = "wordList.txt"

// searchstring: pattern string (smaller string)
// str: container string (larger string)
const getIndicesOfZFunction = ((searchStr, str) => {
  const s = searchStr + "" + str //  is the non-appearing character in str
  let l = 0, r = 0
  const z = [-1]
  const result = []
...
const crawl = (array, idx, pattern, level, src, callback) => {
  ...
  if (level === pattern.length) {
    if (idx === 0) {
        fs.writeFile(src + "/" + outputname, "")
    }

    fs.appendFile(src + "/" + outputname, array[idx] + "\n")
  ...
  https.get(root + array[idx], res => {
    ...
    res.on('end', () => {
      ...
      try {
        fs.mkdirSync(src + "/" + idx)
      } catch (err) {}

      const thisCallback = (() => {crawl(array, idx + 1, pattern, level, src, callback)})
