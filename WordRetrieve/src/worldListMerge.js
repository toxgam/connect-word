...
const root = 'https://www.spellingcity.com'
...
//High School
const xs = [
  '/index.php?pa=get_teaching_resources_word_lists_module&group_ids%5B%5D=386213&title=Compound+Words+9th-10th+Grade+Lists',
  '/index.php?pa=get_teaching_resources_word_lists_module&group_ids%5B%5D=780230&title=Academic+Vocabulary+High+School+Lists',
  '/index.php?pa=get_teaching_resources_word_lists_module&group_ids%5B%5D=386843&title=Heteronyms+High+School+Lists',
  '/index.php?pa=get_teaching_resources_word_lists_module&group_ids%5B%5D=386804&title=Homonyms+9th-12th+Grade+Lists',
  '/index.php?pa=get_teaching_resources_word_lists_module&group_ids%5B%5D=386838&title=Homophones+9th-12th+Grade+Lists',
  '/index.php?pa=get_teaching_resources_word_lists_module&group_ids%5B%5D=247820&title=Informational+Text+9th-12th+Grade+Lists',
  '/index.php?pa=get_teaching_resources_word_lists_module&group_ids%5B%5D=386262&title=Synonyms+%26+Antonyms+9th-12th+Grade+Lists',
  '/index.php?pa=get_teaching_resources_word_lists_module&group_ids%5B%5D=108387&group_ids%5B%5D=108388&group_ids%5B%5D=108390&group_ids%5B%5D=393403&title=Literature+High+School+Lists',
  '/index.php?pa=get_teaching_resources_word_lists_module&group_ids%5B%5D=393147&group_ids%5B%5D=393174&group_ids%5B%5D=32023&group_ids%5B%5D=393165&group_ids%5B%5D=393170&group_ids%5B%5D=393167&title=Math+High+School+Lists',
  '/index.php?pa=get_teaching_resources_word_lists_module&group_ids%5B%5D=387040&group_ids%5B%5D=387712&group_ids%5B%5D=387693&group_ids%5B%5D=387706&group_ids%5B%5D=387703&group_ids%5B%5D=387708&group_ids%5B%5D=387710&group_ids%5B%5D=387700&title=Science+High+School+Lists',
  '/index.php?pa=get_teaching_resources_word_lists_module&group_ids%5B%5D=54055&group_ids%5B%5D=54058&group_ids%5B%5D=54056&group_ids%5B%5D=54057&group_ids%5B%5D=55216&title=Social+Studies+High+School+Lists'
]

const pattern = [{
    pattern: /"(\/view-spelling-list.html\?listId=(.*?))"/g,
    position: 1,
    inject: (template, arr) => arr.map(e => template.substr(e, pattern.length + 40))
  }, {
    pattern: /vsctip.*?>(.+?)</g,
    position: 1,
    inject: (template, arr) => arr.map(e => template.substring(57 + e, template.indexOf("<", 57 + e)))
  }
]

const outputname = "wordListHighSchool.txt"

const crawl = (array, idx, pattern, level, callback) => {
  ...
  if (level === pattern.length) {
    fs.appendFile(outputname, array[idx] + "\n")
    crawl(array, idx + 1, pattern, level, callback)
    
    return 
  }

  https.get(root + array[idx], res => {
    let doc = ""
    ...
    res.on('end', () => {
      ...
      const thisCallback = (() => {crawl(array, idx + 1, pattern, level, callback)})
